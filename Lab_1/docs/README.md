...
[x] 1. **Ctrl-D Handling**: Respond to Ctrl-D (EOF) to exit.

[x] 2. **Basic Commands**: Execute simple commands like `ls`, `date`, and `who`. It should also recognize the `PATH` environment variable to locate commands.

3. **Background Execution**: Allow running commands in the background, e.g., `sleep 30 &`.

[x] 4. **Piping**: Support one or more pipes between commands, e.g., `ls | grep out | wc -w`.
